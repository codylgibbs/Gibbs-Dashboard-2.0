...
export default function Weather({ variant = 'full' }) {
    ...
    const getHeatIndex = (tempF, humidity) => {
        if (tempF < 80)
            return null;
        const T = tempF, RH = humidity;
        return Math.round(-42.379 + 2.04901523 * T + 10.14333127 * RH
            - 0.22475541 * T * RH - 0.00683783 * T * T
            - 0.05391553 * RH * RH + 0.00122874 * T * T * RH
            + 0.00085282 * T * RH * RH - 0.00000199 * T * T * RH * RH);
    };
    const getWindChill = (tempF, windMph) => {
